...
class Weibo(object):
    ...
    def get_json(self, params):
        """获取网页中json数据"""
        url = 'https://m.weibo.cn/api/container/getIndex?'            #这个地址API地址
    ...
    def user_to_mysql(self):
        ...
        # 创建'spider_weibo'数据库
        create_database = """CREATE DATABASE IF NOT EXISTS spider_weibo DEFAULT
                         CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci"""
        self.mysql_create_database(mysql_config, create_database)
        # 创建'user'表
        create_table = """
                CREATE TABLE IF NOT EXISTS user (
                id varchar(20) NOT NULL COMMENT '微博用户id',
                screen_name varchar(30) COMMENT '微博用户昵称' ,
                gender varchar(10)  COMMENT '微博用户性别，取值为f或m，分别代表女和男' ,
                ...
                sunshine varchar(20)  COMMENT '星座' ,
                birthday varchar(40)  COMMENT '生日' ,
                location varchar(200)  COMMENT '地址' ,
                education varchar(200)  COMMENT '教育' ,
                company varchar(200)  COMMENT '公司' ,
                description varchar(140)  COMMENT '微博简介' ,
                profile_url varchar(200)  COMMENT '微博主页' ,
                profile_image_url varchar(200)  COMMENT '微博头像url' ,
                avatar_hd varchar(200)  COMMENT '微博高清头像url' ,
                urank INT  COMMENT '微博等级' ,
                mbrank INT  COMMENT '微博会员等级' ,
                verified BOOLEAN DEFAULT 0  COMMENT '微博是否认证' ,
                verified_type INT  COMMENT '微博认证类型' ,
                verified_reason varchar(140)  COMMENT '微博认证信息' ,
                PRIMARY KEY (id) 
    ...
    def weibo_to_mysql(self, wrote_count):
        ...
        create_table = """
                CREATE TABLE IF NOT EXISTS weibo (
                  ...
                  user_id varchar(20) DEFAULT NULL COMMENT '存储微博用户id',
                  screen_name varchar(30) DEFAULT NULL COMMENT '存储微博昵称',
                  text varchar(2000) DEFAULT NULL COMMENT '存储微博正文',
                  topics varchar(200) DEFAULT NULL COMMENT '存储微博话题，即两个#中的内容。若某条微博没有话题信息，则值为空',
                  at_users varchar(1000) DEFAULT NULL COMMENT '存储微博@的用户。若某条微博没有@的用户，则值为空',
                  pics varchar(3000) DEFAULT NULL COMMENT '存储原创微博的原始图片url。若某条微博有多张图片，则存储多个url，以英文逗号分割；若该微博没有图片，则值为空',
                  video_url varchar(1000) NOT NULL COMMENT '存储原创微博的视频url和Live Photo中的视频url。若某条微博有多个视频，则存储多个url，以英文分号分割；若该微博没有视频，则值为空',
                  location varchar(100) DEFAULT NULL COMMENT '存储微博的发布位置。若某条微博没有位置信息，则值为空',
                  created_at datetime DEFAULT NULL COMMENT '存储微博的发布位置。若某条微博没有位置信息，则值为空',
                  source varchar(30) DEFAULT NULL COMMENT '存储微博的发布工具',
                  attitudes_count int(11) DEFAULT NULL COMMENT '存储微博获得的点赞数',
                  comments_count int(11) DEFAULT NULL COMMENT '存储微博获得的评论数',
                  reposts_count int(11) DEFAULT NULL COMMENT '存储微博获得的转发数',
                  retweet_id varchar(20) DEFAULT NULL COMMENT '存储转发微博中原始微博的微博id',
                  PRIMARY KEY (id)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
                """
        self.mysql_create_table(mysql_config, create_table)
